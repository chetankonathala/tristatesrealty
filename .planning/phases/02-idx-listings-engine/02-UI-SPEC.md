...
Exceptions: map marker touch targets enforce 44px even though content circle may render at 32px (invisible hit area extends to 44px).
...
| Detail page price (hero line) | Display | 48px / 32px mobile | 700 | Playfair, tabular-nums, `--foreground` (NOT gold — detail price is informational hero, not CTA) |
| Detail page address (below price) | Heading | 28px / 24px mobile | 700 | Playfair |
...
[Split layout: SearchFilters (320px) | Results column | SearchMap (40% min 480px)]
...
[ListingHero — 1280px max-width, 2xl (48px) padding-top]
  ...
  - Mobile: stacks, sticky contact bar at bottom
[2xl gap]
...
| Save button (signed out) | triggers sign-in modal with "Sign in to save homes" heading |
